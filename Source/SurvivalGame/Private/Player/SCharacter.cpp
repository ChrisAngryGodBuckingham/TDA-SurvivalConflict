	...
	: Super(ObjectInitializer)
{
	...
	FollowCameraComp = ObjectInitializer.CreateDefaultSubobject<UCameraComponent>(this, TEXT("FollowCamera"));
	FollowCameraComp->SetupAttachment(CameraBoomComp);

	FPCameraComp = ObjectInitializer.CreateDefaultSubobject<UCameraComponent>(this, TEXT("FPCamera"));
	FPCameraComp->SetRelativeLocation(FVector(0.0f, 0.0f, 32.0f ));
	FPCameraComp->bUsePawnControlRotation = true;
	FPCameraComp->SetupAttachment(GetCapsuleComponent());

	MMCameraComp = ObjectInitializer.CreateDefaultSubobject<UCameraComponent>(this, TEXT("MiniMapCamera"));
	MMCameraComp->SetRelativeLocation(FVector(0.0f, 0.0f, 3000.0f));
	MMCameraComp->SetRelativeRotation(FRotator(90.0f, -90.0f, 0.0f));
	MMCameraComp->bUsePawnControlRotation = false;
	MMCameraComp->SetupAttachment(GetMesh());
	...
	HatMesh->SetOwnerNoSee(true);

	TopMesh = CreateDefaultSubobject<USkeletalMeshComponent>(TEXT("TopMesh"));
	TopMesh->SetupAttachment(GetMesh());
	TopMesh->SetOwnerNoSee(true);

	PantMesh = CreateDefaultSubobject<USkeletalMeshComponent>(TEXT("PantMesh"));
	PantMesh->SetupAttachment(GetMesh());
	PantMesh->SetOwnerNoSee(true);
...
void ASCharacter::BeginPlay()
{
	...
	PantMesh->SetMasterPoseComponent(GetMesh());
...
/*Lots overridden here - Sprinting - Usables - Hide in shadows*/
void ASCharacter::Tick(float DeltaTime)
{
	Super::Tick(DeltaTime);

	// Reset hide in shadows light detection
	bIsInAnyLight = false;
					...
					if (Distance > Light->PointLightComponent->AttenuationRadius)
						{
							...
							FHitResult Hit;
							if (GetWorld()->LineTraceSingleByChannel(Hit, EyesLocation, Light->GetActorLocation(), ECC_Visibility, QuereyParams))
								{
									bIsInAnyLight = true;
									break;
								}
